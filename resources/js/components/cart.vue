            ...
            <v-simple-table>
              <tbody>
                <tr v-for="item in $store.state.cart" :key="item.id">
                  <td><v-img :src="`/storage/products/${item.photo_url}`" height="150" width="150"></v-img></td>
                  <td>
                    <span class="d-block"><h5>{{ item.name }}</h5></span>
                  ...
                  <td><v-btn @click.prevent="removeFromCart(item)" icon><v-icon>mdi-close</v-icon></v-btn></td>
                </tr>
              </tbody>
            </v-simple-table>
            <v-row>
              <v-col>
                <div class="text-h6 text-left font-weight-bold">Order total: {{ totalPrice }}€</div>
              </v-col>
              <v-col class="text-right">
                <v-btn color="error" @click.prevent="$store.commit('clearCart')">Clear cart</v-btn>
